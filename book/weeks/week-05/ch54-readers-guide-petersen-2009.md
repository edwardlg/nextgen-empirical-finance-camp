...
**The gap to two-way clustering.** This is a *temporal* vulnerability — a thing the paper could not fully resolve because the tool arrived later. Petersen handles "both effects present" with the methods available in 2009, but the clean, general **two-way cluster-robust estimator** was formalized by Cameron, Gelbach & Miller (2011) and Thompson (2011), *after* Petersen's main contribution. [CHECK: the exact division of labor and dates between Petersen's own two-way treatment, Thompson, and Cameron–Gelbach–Miller — verify who published the general estimator when before attributing it precisely.] So a 2026 reader should treat Petersen's "both" guidance as *correct in spirit* but should reach for the modern two-way estimator (the firm-clustered sandwich plus the time-clustered sandwich minus the White sandwich, from Ch 2.4 §6) for the implementation, and should remember that two-way clustering *also* needs enough clusters in *both* dimensions.
